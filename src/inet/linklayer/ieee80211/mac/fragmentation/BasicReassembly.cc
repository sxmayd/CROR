 ...
 */
Ieee80211DataOrMgmtFrame *BasicReassembly::addFragment(Ieee80211DataOrMgmtFrame *frame) // important function which to decapsulation
{
    // Frame is not fragmented
    if (!frame->getMoreFragments() && frame->getFragmentNumber() == 0)
        return frame;
    // FIXME: temporary fix for mgmt frames
    if (dynamic_cast<Ieee80211ManagementFrame*>(frame)) // frame is a management_frame
        return frame;
    // find entry for this frame
    Key key; // a structure include macAddress... @myd
