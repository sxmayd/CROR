...
void Dcf::processUpperFrame(Ieee80211DataOrMgmtFrame* frame)
{
    ...
    if (pendingQueue->insert(frame)) {
        EV_INFO << "Frame " << frame->getName() << " has been inserted into the PendingQueue." << endl; // do
        EV_DETAIL << "Requesting channel" << endl; // do
...
void Dcf::processLowerFrame(Ieee80211Frame* frame)
{
    ...
    take(frame); // take-over the owner of the msg @myd
...
void Dcf::recipientProcessReceivedFrame(Ieee80211Frame* frame) // if (isForUs(frame)) @myd
{
